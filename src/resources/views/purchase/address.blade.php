...
<form class="edit-address__form" action="{{ route('purchase.address.update', $item) }}" method="POST">
    ...
    <div class="edit-address__form-item">
        <label>郵便番号</label>
        <input type="text" class="edit-address__item-input" name="postal_code" value="{{ old('postal_code', optional($address)->postal_code) }}">
        @error('postal_code') <p class="form__error">{{ $message }}</p> @enderror
    </div>
    <div class="edit-address__form-item">
        <label>住所</label>
        <input type="text" class="edit-address__item-input" name="address" value="{{old('address',optional($address)->address)}}">
        @error('address') <p class="form__error">{{ $message }}</p> @enderror
    </div>
    <div class="edit-address__form-item">
        <label>建物名</label>
        <input type="text" class="edit-address__item-input" name="building" value="{{old('building',optional($address)->building)}}">
